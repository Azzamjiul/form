...
import type { WhitelistEntryListItem, CreateWhitelistRequest } from '../types';
...
export const WhitelistManagement = ({ formId }: WhitelistManagementProps) => {
  ...
  const handleRevokeEntry = async (whitelistId: string) => {
    if (!confirm('Are you sure you want to revoke this access? The user will no longer be able to take the quiz.')) {
      return;
    }
  ...
  const getStatusBadge = (entry: WhitelistEntryListItem) => {
    if (entry.is_expired) {
      return <span className="px-2 py-1 text-xs font-medium text-red-700 bg-red-100 rounded-full">Expired</span>;
    }
    if (entry.attempts_used >= entry.max_attempts) {
      return <span className="px-2 py-1 text-xs font-medium text-orange-700 bg-orange-100 rounded-full">Exhausted</span>;
    }
    ...
    return <span className="px-2 py-1 text-xs font-medium text-gray-700 bg-gray-100 rounded-full">Inactive</span>;
  };
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">Manage who can access this form</p>
        </div>
        <button
      ...
      {isLoading && entries.length === 0 ? (
        ...
        <div className="text-center py-8 border-2 border-dashed border-gray-300 rounded-lg">
          ...
          <p className="text-sm text-gray-400 mt-1">Add respondents to give them access to this form</p>
        </div>
      ) : (
        <>
                ...
                {entries.map((entry) => (
                  <tr key={entry.whitelist_id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                      <button
                        onClick={() => copyToClipboard(`http://localhost:3000/quiz/${entry.whitelist_id}`, entry.whitelist_id)}
                        className="text-blue-600 hover:text-blue-900"
                      >
                        {copiedToken === entry.whitelist_id ? 'Copied!' : 'Copy Link'}
                      </button>
                      <button
